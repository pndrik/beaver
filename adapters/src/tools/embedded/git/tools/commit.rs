...
const NAME: &str = "git_commit";
const DESCRIPTION: &str =
    "Stages all changes and creates a commit in an existing local repository.";
...
impl Tool for Commit {
    ...
    async fn call(
        ...
        git_repository.commit(&args.message).map_err(|e| {
            app_error!(
                Internal,
                "tool_failed",
                &format!("Failed to commit in repository '{}': {}", args.path, e),
                ctx.clone()
            )
        })?;

        Ok(format!("Committed changes in repository '{}'", args.path))
    }
}
