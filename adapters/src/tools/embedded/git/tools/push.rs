...
const NAME: &str = "git_push";
const DESCRIPTION: &str = "Pushes the current branch of an existing local repository to its remote and sets upstream tracking.";
...
impl Tool for Push {
    ...
    async fn call(
        ...
        git_repository.push().map_err(|e| {
            app_error!(
                Internal,
                "tool_failed",
                &format!("Failed to push repository '{}': {}", args.path, e),
                ctx.clone()
            )
        })?;

        Ok(format!("Pushed repository '{}'", args.path))
    }
}
